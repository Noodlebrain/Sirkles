...
;Backbutton graphic
(define backButtonGraphic (bitmap/file "graphics/backbutton.png"))
...
;Menu buttons
(define button1graphic (bitmap/file "graphics/button1.png"))
(define button2graphic (bitmap/file "graphics/button2.png"))
(define button3graphic (bitmap/file "graphics/button3.png"))
(define button4graphic (bitmap/file "graphics/button4.png"))
...
(define (addOffset beatmap offset)
    ...
    [else (cons
           ...
           (addOffset (rest beatmap) offset))]))

;Creating the beatmap for the world
(define currentBeatmap (makeMap "beatmaps/map1.txt"));Map to a list of notes
...
(define backButton ;backButton
  (make-button (make-region 0 (- cvHeight 174) 450 cvHeight)
                 (make-world 0 (make-posn 0 0) 0 '() )
                 ding))

(define button1
  (make-button (make-region (- cvLength 362) 0 cvLength 127)
                 (make-world 3 (make-posn 1 0) 0 beatmap1)
                 song1))

(define button2
  (make-button (make-region (- cvLength 362) 127 cvLength 254)
                 (make-world 3 (make-posn 2 0) 0 beatmap2)
                 song2))

(define button3
  (make-button (make-region (- cvLength 362) 254 cvLength 381)
                 (make-world 3 (make-posn 3 0) 0 beatmap3)
                 song3))

(define button4
  (make-button (make-region (- cvLength 362) 381 cvLength 508)
...
(define (drawhandler3 w)
  (place-images
   (list cursor
         (text (string-append "Current score: " (number->string (world-score w))) 20 "white"))
   (list (world-posn w) (make-posn (- cvLength 115) 25))
