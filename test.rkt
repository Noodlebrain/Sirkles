...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname test) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp")) #f)))
...
(define pen1 (make-pen (color 75 75 210 204) 10 "solid" "round" "round"))
...
(define (noOfCircles n)
  (cond [(= 0 n) '()]
        [else (cons (circle cRad "solid" (color (random 256) (random 256) (random 256) (random 256))) (noOfCircles (sub1 n)))]))
...
(define (approachCircles beatmap)
  (cond
    [(empty? beatmap) '()]
    [else (cons (circle
                 (+ cRad (* 1.2 cRad (/ (+ (- (note-time (first beatmap)) (pstream-current-frame ps)) timingLenience) (s .5))))
